...
public class RedstoneCeilingTorchBlock extends CeilingTorchBlock {
    public static final BooleanProperty LIT;
    private static final Map<BlockView, List<RedstoneCeilingTorchBlock.BurnoutEntry>> BURNOUT_MAP;

    public RedstoneCeilingTorchBlock(int lightLevel, ParticleEffect particle) {
        super(lightLevel, particle);
    ...
    public int getWeakRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        return state.get(LIT) && Direction.DOWN != direction ? 15 : 0;
    }
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        while(list != null && !list.isEmpty() && world.getTime() - ((RedstoneCeilingTorchBlock.BurnoutEntry)list.get(0)).time > 60L) {
            list.remove(0);
        }
    ...
    private static boolean isBurnedOut(World world, BlockPos pos, boolean addNew) {
        List<RedstoneCeilingTorchBlock.BurnoutEntry> list = BURNOUT_MAP.computeIfAbsent(world, (blockView) -> Lists.newArrayList());
        if (addNew)
            list.add(new RedstoneCeilingTorchBlock.BurnoutEntry(pos.toImmutable(), world.getTime()));
